...
from app.schemas.product import TopProductResponce
from app.services.product import get_top_products, get_images_path_by_product_id
...
def get_top_products_route(limit: int = 10, db: Session = Depends(get_db)):
    ...
    for product in products:
        images_path = get_images_path_by_product_id(db, product.id)
        
        variant = product.variants[0] if product.variants else None
        result.append(TopProductResponce(
            id=product.id,
            name=product.name,
